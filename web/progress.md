...
- Added a synth-first game audio helper in `web/helpers/gameSynth.js` that uses Web Audio to generate short retro piezo-style beeps with slight per-button pitch changes.
...
- Reworked the main `MEDICINE` action into a stock-free `HEAL` action: it no longer consumes shop inventory, the shop no longer sells medicine, and legacy saves with `inventory.medicine` remain loadable but that value is now ignored.
- Added reusable `reaction-happy` and `reaction-angry` action animations for heal outcomes, and wired `UIScene` so successful treatment shows the happy reaction while unnecessary treatment shows the angry reaction instead of a message popup.
- Hardened `web/helpers/menuFormatters.js` so dead-menu/status rendering no longer crashes on missing objects after reload, and added `web/tests/menuFormatters.test.mjs` coverage for undefined item/effect payloads.
- Fixed dead-state input handling in `web/scenes/UIScene.js` so `cancel` stays inside the dead menu, `OK` reliably triggers `NEW EGG`, and restart now guards against duplicate presses while resetting UI state before the async scene reload.
- Added lifecycle guards around `web/scenes/GameScene.js#setMenuVisible()` and `UIScene.renderScreenMenu()` so the dead-refresh-new-egg flow no longer throws while `GameScene` is being stopped/restarted.
- Verified syntax with `node --check` for `web/helpers/menuFormatters.js`, `web/scenes/UIScene.js`, `web/scenes/GameScene.js`, ran inline Node assertions for the formatter guards, and replayed the exact browser sequence (`DEBUG > DEAD` equivalent saved state -> refresh -> dead menu `cancel` -> `NEW EGG`) with Playwright; the app now returns to a live `egg` state with no page errors.
